...
class Wiley(Workflow):
    ...
    def prepare_batch(self, *, synced: bool = False) -> tuple[list, ...]:  # noqa: ARG002
        ...
        s3_client.download_file(
            s3_uri=self._get_current_wiley_csv_uri(
                batch_location=f"{CONFIG.s3_bucket_submission_assets}/{original_batch_path}"
            ),
    ...
    def _get_current_wiley_csv_uri(self, batch_location: str) -> str:
        """Get current input CSV file of DOIs from batch.

        NOTE: Currently this method is set to a hardcoded value. This method
        will be updated after discussions with Wiley on upload specifics
        and formatting.
        """
        filename = "MIT_Authored_Articles_Wiley.csv"
        return f"s3://{batch_location}{filename}"
